...
[show.mp4](/paper/show.mp4)
<!--## 前置知识
> TODO-->

## 3D场景重建
概念：通过对同一个物体的不同角度的照片构建出3D模型
...
### 前置
- 人如何看世界：光照射到物体上，物体反射光线到人眼，人眼捕捉光线，大脑处理信息，形成视觉
- 相机如何照相：光照射到物体上，物体反射光线到相机，光线聚焦在感光元件上，感光元件将光信号转化为电信号，通过图像处理器处理后，存储为数字照片
- 游戏：设计3D场景，随玩家移动，将游戏中的假想光源和景象反射到游戏的接收点
- 渲染：将3D模型转换为在屏幕上看到的图片/影片
  - Ray Tracing
  - Rasterization
<!--{{< youtube LLSLSRgoIjY >}}-->

通过算法构建真实世界或虚拟场景的数字化3D表示，实现高效、高质量的动态视角合成，核心是将2D观测数据（多张图片）转化为计算机可理解的3D结构表示

- 核心逻辑：从 2D 到 3D 的 “逆过程”
  现实：3D 物体→光线→相机 / 人眼→2D 图像（如手机拍的街景照）
  技术目标：多张不同角度的 2D 图像→算法计算→还原 3D 场景结构（包含物体位置、形状、颜色）→生成新视角 2D 图像（如虚拟漫游时的画面）
- 两种核心表示技术
  - 显示表示
  - 隐式表示
...
## 论文解读
> 提出分层 3D 高斯表示，突破现有技术在超大场景（如数公里城市街景、数万张图像规模）下的渲染瓶颈，在保证照片级视觉质量的同时，实现实时新视角合成（＞30 FPS） ，解决 “场景规模 - 渲染效率 - 视觉质量” 三者难以平衡的核心问题。
